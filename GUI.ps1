...
function Check-ForUpdate {
    ...
    $lb.AppendText("`n[INIT] Welcome to Windows Maintenance Tool v$AppVersion`n")
    $lb.AppendText("[UPDATE] Checking for updates...`n")
    
    $localVer = [Version]$script:AppVersion
    $updateUrl = "https://raw.githubusercontent.com/Chaython/Windows-Maintenance-Tool/refs/heads/main/gui.ps1?t=$((Get-Date).Ticks)"
    ...
    try {
        ...
        if ($remoteContent -match '\$AppVersion\s*=\s*"(\d+(\.\d+)+)"') {
            ...
            if ($remoteVer -gt $localVer) {
                 $lb.AppendText("[UPDATE] Update found: v$remoteVer!`n")
                 if ([System.Windows.MessageBox]::Show("New version v$remoteVer available. Update?", "Update Found", [System.Windows.MessageBoxButton]::YesNo) -eq "Yes") {
                ...
                $lb.AppendText("[UPDATE] Client is up to date.`n") 
            }
        } else {
            $lb.AppendText("[UPDATE] Could not parse remote version.`n")
        }
    } catch { 
        $lb.AppendText("[UPDATE] Could not check for updates (Offline?).`n") 
    }
    $lb.ScrollToEnd()
}
        ...
        <Border Grid.Column="1" Background="#121212">
            <Grid Margin="20">
                ...
                <StackPanel Name="pnlSupport" Visibility="Collapsed">
                    ...
                    <TextBlock Text="Windows Maintenance Tool v8.8" FontSize="16" FontWeight="Bold"/>
...
$btnResetWifi.Add_Click({ Run-Cmd { Get-NetAdapter | Where-Object { $_.Name -like "*Wi-Fi*" } | Restart-NetAdapter } })
...
$btnHostsBackup.Add_Click({ Run-Cmd { Copy-Item "$env:windir\System32\drivers\etc\hosts" "$env:USERPROFILE\Desktop\hosts_bk.bak"; "Backup on Desktop" } })
...
$btnSFC.Add_Click({ Start-Process cmd "/k sfc /scannow" })
