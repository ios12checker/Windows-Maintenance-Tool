...
echo ======================================================
echo           WINDOWS MAINTENANCE TOOL V2.9.9 - By Lil_Batti
echo ======================================================
echo.
...
setlocal EnableDelayedExpansion
set "ADAPTERLIST="
for /f "skip=1 tokens=* delims=" %%a in ('wmic nic where "NetEnabled=true" get NetConnectionID ^| findstr /R /V "^$"') do (
    if not "%%a"=="NetConnectionID" (
...
REM Try English and Danish standard names
set "RESTARTED="

for %%N in ("Wi-Fi" "Wi-Fi 2" "Wireless network connection" "WLAN" "WLAN 2") do (
    netsh interface set interface name=%%N admin=disable >nul 2>&1
    timeout /t 2 >nul
    netsh interface set interface name=%%N admin=enable >nul 2>&1
    REM Check if adapter exists by querying status
    netsh interface show interface name=%%N >nul 2>&1 && set "RESTARTED=1"
)

if not defined RESTARTED (
    echo No Wi-Fi adapters with common names found.
    echo Try the advanced option or check your adapter name.
) else (
    echo Wi-Fi adapters restarted.
)
echo.
pause
endlocal
